...
public class ProcesoPanel extends JPanel {
    ...
    public ProcesoPanel(GestionProcesos gestionProcesos) {
        ...
        // Personalizar color del botón salir
        salirButton.setFont(new Font("Segoe UI", Font.BOLD, 11));
        ...
        salirButton.setPreferredSize(new Dimension(60, 25));

        // Agregar botones principales al panel izquierdo
        buttonPanel.add(crearButton);
        buttonPanel.add(eliminarButton);

        // Panel específico para el botón salir
        JPanel salirPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        salirPanel.setOpaque(false);
        salirPanel.add(salirButton);

        // Agregar el botón salir al lado derecho del panel inferior
        bottomPanel.add(buttonPanel, BorderLayout.CENTER);
        bottomPanel.add(salirPanel, BorderLayout.SOUTH);
        ...
        salirButton.addActionListener(e -> {
            int confirmacion = showConfirmDialog("¿Está seguro que desea salir?");
            if (confirmacion == JOptionPane.YES_OPTION) {
